...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const safeName = escapeHtml((candidateName || "Unknown").slice(0, 200));
    const safeEmail = escapeHtml((candidateEmail || "").slice(0, 200));
    ...
    await client.send({
      from: SMTP_FROM!,
      to: notifyEmail,
      subject: `New Candidate Resume: ${safeName}`,
      html: `
        <div style="font-family: Inter, sans-serif; max-width: 500px; margin: 0 auto;">
          ...
          <div style="padding: 24px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 8px 8px;">
            <h2 style="margin: 0 0 16px; color: #111827;">New Resume Submission</h2>
            <p style="color: #4b5563; line-height: 1.6;">
              A new candidate has submitted their resume through the "Become a Candidate" form.
            </p>
            ...
            <p style="color: #4b5563; line-height: 1.6;">
              Log in to the admin panel to review the submission.
            </p>
          </div>
        </div>
      `,
